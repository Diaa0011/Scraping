...
class sfmaSpider(scrapy.Spider):
    ...
    def parse_details(self,response):
        ...
        items = SfmaItem()
        items['Name'] = response.css('.text-sfma ::text').get()
        items['Address'] = ', '.join(address_parts)
        items['Phone'] = sh.cleanup(response.xpath('//strong[text()="Tel:"]/../text()').get())
        items['Fax'] = sh.cleanup(response.xpath('//strong[text()="Fax:"]/../text()').get())
        items['Email'] = sh.cleanup(response.xpath('//strong[text()="Email:"]/../text()').get())
        items['Website'] = sh.cleanup(response.xpath('//strong[text()="Website:"]/../text()').get())
        yield items
